...
export default function ProjectDetailPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [projectData, buildsData, featuresData] = await Promise.all([
          projectsApi.getOne(projectId, user.id),
          buildsApi.getAll(user.id, projectId),
          featuresApi.getAll()
        ])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      if (!user?.id) return
      try {
        const updatedBuild = await buildsApi.getOne(activeBuild.id, user.id)
        ...
        if (updatedBuild.status === 'completed') {
          ...
          // Refresh builds list
          const buildsData = await buildsApi.getAll(user.id, projectId)
          setBuilds(buildsData)
  ...
  const handleFeatureToggle = async (featureId: string, enabled: boolean) => {
    ...
    try {
      await projectsApi.update(projectId, { features: updatedFeatures }, user.id)
